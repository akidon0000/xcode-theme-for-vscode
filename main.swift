...
struct EditorPalette {
    ...
    static let xcode11DefaultDark = EditorPalette(
    ...
    static let xcode11DefaultLight = EditorPalette(
        fg: (0x000000, 255),
        bg: 0xFFFFFF,
        selection: 0xB2D7FF,
        cursor: 0x000000,
...
func main() throws {
    ...
    generateEditorColors(builder: darkBuilder, palette: EditorPalette.xcode11DefaultDark)
    let darkTheme = darkBuilder.build(name: "Xcode 11 Default Dark")
    try saveTheme(darkTheme, filename: "Xcode 11 Default Dark-color-theme.json")

    // Generate Light Theme
    let lightBuilder = ThemeBuilder()
    generateUIColors(builder: lightBuilder, palette: UiPalette.light)
    generateEditorColors(builder: lightBuilder, palette: EditorPalette.xcode11DefaultLight)
    let lightTheme = lightBuilder.build(name: "Xcode 11 Default Light")
    try saveTheme(lightTheme, filename: "Xcode 11 Default Light-color-theme.json")
}
